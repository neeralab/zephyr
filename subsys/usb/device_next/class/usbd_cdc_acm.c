...
void cdc_acm_uart_tx_discard(const struct device *dev)
{
	...
	(void)k_work_cancel_delayable(&data->tx_fifo_work);
	atomic_clear_bit(&data->state, CDC_ACM_TX_FIFO_BUSY);
...
void cdc_acm_uart_tx_drain(const struct device *dev, int timeout_ms)
{
	...
	while (elapsed < timeout_ms) {
		const bool busy = atomic_test_bit(&data->state, CDC_ACM_TX_FIFO_BUSY);
		const bool empty = ring_buf_is_empty(data->tx_fifo.rb) && !data->zlp_needed;
		...
		if (!busy && (!empty || data->zlp_needed)) {
			cdc_acm_work_schedule(&data->tx_fifo_work, K_NO_WAIT);
			(void)k_work_flush_delayable(&data->tx_fifo_work, &sync);
		}
